...
class MCP4728 {
 public:
  ...
  void enable(bool b) { digitalWrite(pin_ldac_, !b); }
  ...
  uint8_t analogWrite(uint16_t a, uint16_t b, uint16_t c, uint16_t d,
                      bool b_eep = false) {
    if (b_eep) {
    ...
    if (wire_->available() == 24) {
      for (uint8_t i = 0; i < 8; ++i) {
        ...
        if (isEeprom) {
          ...
          read_eep_[ch].data =
              (uint16_t)((data[1] & 0b00001111) << 8 | data[2]);
        } else {
 ...
 private:
  ...
  uint8_t multiWrite() {
    ...
    for (uint8_t i = 0; i < 4; ++i) {
      ...
      wire_->write(((uint8_t)reg_[i].vref << 7) | ((uint8_t)reg_[i].pd << 5) |
  ...
  uint8_t seqWrite() {
    ...
    for (uint8_t i = 0; i < 4; ++i) {
      wire_->write(((uint8_t)eep_[i].vref << 7) | ((uint8_t)eep_[i].pd << 5) |
                   ((uint8_t)eep_[i].gain << 4) | highByte(eep_[i].data));
  ...
  uint8_t singleWrite(uint8_t ch) {
    ...
    wire_->write(((uint8_t)eep_[ch].vref << 7) | ((uint8_t)eep_[ch].pd << 5) |
                 ((uint8_t)eep_[ch].gain << 4) | highByte(eep_[ch].data));
